...
DWORD CreateThresholdBarrier(THB_OBJECT *pthb , DWORD b_value)
{
	...
	*pthb = hthb;
	...
	/* TODO - Create Mutex */
	barrier->hGuard = CreateMutex (
		NULL ,  /* default security attributes */
		FALSE , /* initially not owned */
		NULL  /* unnamed mutex */
	);
	DIE(barrier->hGuard == NULL , "CreateMutex");

	/* TODO - Create Event */
	barrier->hEvent = CreateEvent (
		NULL ,
		TRUE ,  /* Manual Reset */
		FALSE , /* Non-signaled state */
		NULL   /* Private variable */
	);
	DIE (barrier->hEvent == NULL , "CreateEvent");
...
DWORD WaitThresholdBarrier(THB_OBJECT thb)
{
	...
	bRet = WaitForSingleObject (thb->hGuard , INFINITE);
	//DIE(bRet == FALSE , "WaitForSingleObject");
	...
	if (thb->dwCount == thb->dwThreshold) {
		...
		SignalObjectAndWait (thb->hGuard , thb->hEvent , INFINITE ,
				     FALSE);
	}
